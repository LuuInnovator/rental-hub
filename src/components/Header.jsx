...
const Header = ({ searchTerm, setSearchTerm, adminName, toggleSidebar }) => {
  // Lấy chữ cái đầu tiên của tên để làm Avatar
  const avatarLetter = adminName ? adminName.charAt(0).toUpperCase() : "A";

  return (
    <header className="bg-white/80 backdrop-blur-md shadow-sm h-20 flex items-center justify-between px-6 md:px-10 fixed top-0 left-0 md:left-64 right-0 z-40 border-b border-gray-100/50">
      
      <div className="flex items-center gap-4 flex-1">
        {/* Nút Menu chỉ hiện trên Mobile (dưới 768px) */}
        <button 
          onClick={toggleSidebar}
          className="p-2 text-gray-600 hover:bg-gray-100 rounded-xl md:hidden transition-colors"
        >
      ...
      <div className="flex items-center gap-3 md:gap-6">
        {/* Notifications - Ẩn bớt icon phụ trên mobile cực nhỏ nếu cần, ở đây giữ nguyên vì gap đã thu nhỏ */}
        ...
        {/* User Profile */}
        <div className="flex items-center gap-3 md:gap-4 border-l border-gray-100 pl-4 md:pl-6 h-10">
          ...
          <div className="relative group cursor-pointer">
            <div className="w-10 h-10 md:w-11 md:h-11 bg-gradient-to-tr from-blue-600 to-blue-400 rounded-2xl flex items-center justify-center font-black text-white shadow-lg shadow-blue-100 group-hover:scale-105 transition-transform uppercase">
              {avatarLetter}
            </div>
